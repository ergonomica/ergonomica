...
def prompt(env, ns):
    ...
    return prompt_toolkit.prompt(unicode(env.prompt.replace("<user>", env.user).replace("<directory>", env.directory)), multiline=True, completer=ErgonomicaCompleter(ns), history=HISTORY)
